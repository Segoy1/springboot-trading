...
public class ApiResponseKafkaHandler {
  ...
  @KafkaListener(
      ...
      topics = "${kafka.names.topic.errorMessage}")
  ...
  public void consumeMessage(ProfitAndLossData message) {
    ...
    messagingTemplate.convertAndSend(
        "/topic/" + kafkaConstantsConfig.getSINGLE_PNL_TOPIC(), message);
  }
  ...
  @KafkaListener(
      groupId = "${kafka.consumer.group.id}",
      topics = "${kafka.names.topic.accountSummary}")
  ...
  @KafkaListener(
      groupId = "${kafka.consumer.group.id}",
      topics = "${kafka.names.topic.optionMarketData}")
  ...
  public void consumeMessage(PositionData message) {
    if (message.getContractData().getSecurityType().equals(Types.SecType.BAG)) {
      ...
      streamsAggregatedPositionHandler
          .persistContractAndPositionData(message)
          .ifPresent(
              (updatedPosition) ->
                  messagingTemplate.convertAndSend(
      ...
      messagingTemplate.convertAndSend(
          "/topic/" + kafkaConstantsConfig.getPOSITION_TOPIC(), message);
    }
  }
}
