...
public class UniqueContractDataProvider {
    ...
    private Optional<ContractDbo> getComboLegOptionData(ContractDbo contractDBO) {
        ...
        OptionalLong id = comboContractDataFinder.checkContractWithComboLegs(contractDBO.getComboLegs());
        if (id.isPresent()) {
    ...
    private Optional<ContractDbo> getIndexOrStockData(ContractDbo contractDBO) {
        Optional<ContractDbo> contractOpt =
                contractRepository.findFirstBySymbolAndSecurityTypeAndCurrency(contractDBO.getSymbol(),
                        contractDBO.getSecurityType(),
                        contractDBO.getCurrency());
    ...
    private Optional<ContractDbo> getOptionContractData(ContractDbo contractDBO) {
        Optional<ContractDbo> contractOpt = contractRepository.findFirstByLastTradeDateAndSymbolAndStrikeAndRight(
