...
class HistoricalDataDatabaseSynchronizerTest {
    ...
    void testFindInDb(){
        ...
        HistoricalData result =
                HistoricalData.builder().id(1L).close(1.0).contractId(1).low(0.0).open(1.0).high(2.0).volume(BigDecimal.valueOf(100)).count(50).wap(BigDecimal.ZERO).build();
    ...
    void testSaveToDb(){
        ...
        Timestamp time = Timestamp.valueOf(LocalDateTime.now());
        HistoricalData result =
                HistoricalData.builder().close(1.0).low(0.0).open(1.0).high(2.0).volume(BigDecimal.valueOf(100)).count(50).wap(BigDecimal.ZERO).build();
